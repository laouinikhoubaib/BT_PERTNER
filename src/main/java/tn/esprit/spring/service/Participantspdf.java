...
public class Participantspdf {

	private List<Eventparticipants> ev;
    
    public Participantspdf (List<Eventparticipants> ev) {
        this.ev = ev;
    }
    ...
    private void writeTableData(PdfPTable table) {
        for (Eventparticipants user : ev ) {
        	
            table.addCell(String.valueOf(user.getIduser()));
            table.addCell(user.getUserename());
    ...
    public void export(HttpServletResponse response) throws DocumentException, IOException {
        ...
        table.setWidths(new float[] {1.0f, 1.5f, 3.0f});
